        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for x1, y1, x2, y2, conf, cls in reversed(det):
                    ...
                    w = (x2 - x1)/image_w
                    h = (y2 - y1)/image_h
                    xywh_list.append([c, float(x), float(y), float(w), float(h)])
              
            with open(ROOT_p+'/detect/Face_Mask/'+p.stem+'.txt', 'w+') as file:
                for i in range(len(xywh_list)):
                    file.write(' '.join(str(num) for num in xywh_list[i]) + "\n")
